<p align="center">
  <img src="../../assets/logojorvel.png" alt="dashboard" width="120" height="120">
</p>

<h1 align="center">@app/dashboard</h1>

<p align="center">Generated by <a href="https://jorveljs.vercel.app">JORVEL</a> on port <code>3001</code>.</p>
...
├── src/
│   ├── error-boundary.jsx   # top-level React error boundary
│   ├── pages/
│   │   ├── _error.jsx       # dev stack / prod-safe crash screen
│   │   ├── _404.jsx         # default not-found page
│   │   └── ...               # file-based routes (remote)
│   ├── bootstrap.jsx       # wires ErrorBoundary + 404 fallthrough
│   └── main.jsx
├── jorvel.app.json
└── rspack.config.mjs
```
...
- `src/pages/_error.jsx` — shown when the top-level `<ErrorBoundary>` (see `src/error-boundary.jsx`) catches a render error. In `NODE_ENV !== "production"` the full stack is inlined; production renders a brand-safe message.
- `src/pages/_404.jsx` — shown when the current URL is not matched by any entry in `jorvel.routes.host.json`.
...
pnpm dev            # this app on http://localhost:3001 (Rspack dev server + HMR)
```
